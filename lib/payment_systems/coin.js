...
let paymentQueue = async.queue(function (paymentDetails, callback) {
  ...
  global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
    ...
    if (paymentDetails.hasOwnProperty('payment_id')) {
      console.log("Payment made to " + paymentDetails.destinations[0].address + " with PaymentID: " + paymentDetails.payment_id + " For: " + global.support.coinToDecimal(paymentDetails.destinations[0].amount) + " " + global.config.general.coinCode.toUpperCase() + " with a " + global.support.coinToDecimal(body.result.fee) + " " + global.config.general.coinCode.toUpperCase() + " Mining Fee");
      return callback(body.result);
    } else {
      if (transferFunc === 'transfer') {
        console.log("Payment made out to multiple people, total fee: " + global.support.coinToDecimal(body.result.fee) + " " + global.config.general.coinCode.toUpperCase());
      }
      let intCount = 0;
      paymentDetails.destinations.forEach(function (details) {
        console.log("Payment made to: " + details.address + " For: " + global.support.coinToDecimal(details.amount) + " " + global.config.general.coinCode.toUpperCase());
        intCount += 1;
...
function Payee(amount, address, paymentID, bitcoin) {
  ...
  this.makePaymentWithID = function () {
    ...
    paymentQueue.push(paymentDetails, function (body) {
      if (typeof body.tx_hash !== 'undefined') {
        ...
        global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, coin_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
  ...
  this.makePaymentAsIntegrated = function () {
    ...
    paymentQueue.push(paymentDetails, function (body) {
      if (typeof body.tx_hash !== 'undefined') {
        debug("Successful payment sent to: " + identifier);
        global.mysql.query("INSERT INTO transactions (bitcoin, address, coin_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
function makePayments() {
  global.mysql.query("SELECT * FROM balance WHERE amount >= ?", [global.support.decimalToCoin(global.config.payout.walletMin)]).then(function (rows) {
    ...
    rows.forEach(function (row) {
      ...
      // if (row.amount > 1000 * 100000) row.amount = 900 * 100000;
